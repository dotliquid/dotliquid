...
namespace DotLiquid.Tests
{
    ...
    public class CustomIndexableTests
    {
        ...
        {
            string output = Template.Parse("{%for item in list%}{{ item }} {%endfor%}", NamingConvention)
        ...
        {
            string output = Template.Parse("1: {{ list[0] }}, 2: {{ list[1] }}, 3: {{ list[2] }}", NamingConvention)
                .Render(Hash.FromAnonymousObject(new {list = new VirtualList(1, "Second", 3)}));
        ...
        public void TestCustomIndexableRender()
        {
            Assert.AreEqual(
                ...
                actual: Template
                    .Parse("{{container}}", NamingConvention)
        ...
        {
            string output = Template.Parse("1: {{container[0]}}, 2: {{container[1]}}", NamingConvention)
                .Render(Hash.FromAnonymousObject(new {container = new CustomIndexable()}));
        ...
        {
            string output = Template.Parse("1: {{container[2147483648]}}, 2: {{container[2999999999]}}", NamingConvention)
                .Render(Hash.FromAnonymousObject(new { container = new CustomIndexable() }));
        ...
        public void TestCustomIndexableStringKeys() {
            string output = Template.Parse("abc: {{container.abc}}", NamingConvention)
                .Render(Hash.FromAnonymousObject(new {container = new CustomIndexable()}));
