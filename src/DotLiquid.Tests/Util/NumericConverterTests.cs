...
namespace DotLiquid.Tests.Util
{
    ...
    public class NumericConverterTests
    {
        ...
        {
            object coercedValue = NumericConverter.CoerceToNumericType(input, formatProvider, null);
        ...
        public void TestCoerceToNumericTypeErrors(object input, IFormatProvider formatProvider)
        {
            ...
            object coercedValue = NumericConverter.CoerceToNumericType(input, formatProvider, defaultValue);
        ...
        {
            bool converted = NumericConverter.TryCoerceToNumericType(input, formatProvider, out object convertedValue);
        ...
        {
            bool converted = NumericConverter.TryParseToNumericType(input, formatProvider, out object convertedValue);
            Assert.That(converted, Is.True);
            Assert.That(convertedValue, Is.EqualTo(expectedValue));
            Assert.That(convertedValue, Is.TypeOf(expectedValue.GetType()));
        }
        ...
        private static IEnumerable GoodTestCaseSource()
        {
            ...
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0}", 12567),
                invariantFormatProvider,
                12567 };
            yield return new object[] {
                String.Format(frenchFormatProvider, "{0:#,##0}", 12567),
                frenchFormatProvider,
                12567 };
            // Note: For fallback to happen, the number must be big enough to contain 2 separators,
            // otherwise it will be interpreted as a French floating point number.
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0}", 12567890),
                frenchFormatProvider,
                12567890 };
            ...
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0}", Int64.MaxValue),
                invariantFormatProvider,
                Int64.MaxValue };
            yield return new object[] {
                String.Format(frenchFormatProvider, "{0:#,##0}", Int64.MaxValue),
                frenchFormatProvider,
                Int64.MaxValue };
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0}", Int64.MaxValue),
            ...
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0.00}", 12567.1m),
                invariantFormatProvider,
                12567.1m };
            yield return new object[] {
                String.Format(frenchFormatProvider, "{0:#,##0.00}", 12567.1m),
                frenchFormatProvider,
                12567.1m };
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0.00}", 12567.1m),
            ...
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0.00}", largePositiveValue),
                invariantFormatProvider,
                largePositiveValue };
            yield return new object[] {
                String.Format(frenchFormatProvider, "{0:#,##0.00}", largePositiveValue),
                frenchFormatProvider,
                largePositiveValue };
            yield return new object[] {
                String.Format(invariantFormatProvider, "{0:#,##0.00}", largePositiveValue),
        ...
        private static IEnumerable ErrorTestCaseSource()
        {
            ...
            yield return new object[] { "1.023.456", invariantFormatProvider };
            yield return new object[] { "1.023,456", invariantFormatProvider };
        }
    }
}
