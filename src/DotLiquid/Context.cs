...
namespace DotLiquid
{
    public class Context
    {
        ...
        public Context(List<Hash> environments, Hash outerScope, Hash registers, bool rethrowErrors, bool restrictTypes)
        {
        ...
        public Context(bool restrictTypes = true)
            : this(new List<Hash>(), new Hash(), new Hash(), false, restrictTypes)
        {
        }
        ...
        {
            get { return Resolve(key, _restrictTypes); }
            set { Scopes[0][key] = value; }
        }

        public bool HasKey(string key, bool restrictTypes)
        {
            return Resolve(key, restrictTypes) != null;
        }
        ...
        /// <returns></returns>
        private object Resolve(string key, bool restrictTypes)
        {
            switch (key)
            {
                case null:
            ...
            if (match.Success)
                return Range.Inclusive(Convert.ToInt32(Resolve(match.Groups[1].Value, restrictTypes)),
                    Convert.ToInt32(Resolve(match.Groups[2].Value, restrictTypes)));
            ...
            return Variable(key, restrictTypes);
        }
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        private object FindVariable(string key, bool restrictTypes)
        {
            ...
            variable = Liquidize(variable, restrictTypes);
        ...
        /// <param name="restrictTypes"></param>
        /// <returns></returns>
        private object Variable(string markup, bool restrictTypes)
        {
            ...
            if (firstPartSquareBracketedMatch.Success)
                firstPart = Resolve(firstPartSquareBracketedMatch.Groups[1].Value, restrictTypes).ToString();

            object @object;
            if ((@object = FindVariable(firstPart, restrictTypes)) != null)
            {
                foreach (string forEachPart in parts)
                {
                    ...
                    if (partResolved)
                        part = Resolve(partSquareBracketedMatch.Groups[1].Value, restrictTypes);
                    ...
                    if (@object is KeyValuePair<string, object> && ((KeyValuePair<string, object>)@object).Key == (string)part)
                    {
                        ...
                        @object = Liquidize(res, restrictTypes);
                    }
