...
namespace DotLiquid
{
    public class RenderParameters
    {
        ...
        internal void Evaluate(Template template, out Context context, out Hash registers, out IEnumerable<Type> filters, bool restrictTypes)
        {
            if (Context != null)
            {
                context = Context;
            ...
            context = new Context(environments, template.InstanceAssigns, template.Registers, RethrowErrors, restrictTypes);
            registers = Registers;
            filters = Filters;
        }
