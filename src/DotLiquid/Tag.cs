...
namespace DotLiquid
{
    public class Tag : IRenderable
    {
        ...
        public virtual void Render(Context context, TextWriter result, bool restrictTypes = true)
        {
        }
        ...
        /// <returns></returns>
        internal string Render(Context context, bool restrictTypes = true)
        {
            using (TextWriter result = new StringWriter())
            {
                Render(context, result, restrictTypes);
